...
public final class IpClient {
	...
	public IpClient() {
		...
		if (!initClientPrivateKey(IpConfig.ipConfig().getClientPrivateKey())
				| !initServerPublicKey(IpConfig.ipConfig().getServerPublicKey())
				| !initClientPublicKey(IpConfig.ipConfig().getClientPublicKey())
				| !initCredentials(IpConfig.ipConfig().getCredentials())) {
			return;
		}
	 ...
	 * @param random2  random value used to salt credentials field.
	 ...
	 */
	private boolean buildServerMessageData(Map<String, String> data, String random1, String random2, String publicIp) {
		...
		buffer.append(_user);
		buffer.append('\n');
		buffer.append(random2);
		buffer.append('\n');
		buffer.append(_password);

		data.put(IpConfig.CONSTANT_HTTP_KEY_CREDENTIALS, encryptData(_serverPublicKey, buffer.toString()));
	 ...
	 * server's {@link java.security.PublicKey}. {@link initServerPublicKey(String)}
	...
	private boolean getPublicIpFromServer(StringBuilder ipOut) {
		...
		try {
			...
			if (!buildServerMessageData(requestData, random, getRandom(), "")) {
	 ...
	 */
	private boolean initClientPrivateKey(String clientPrivateKeyFilename) {
	 ...
	 * file and stores the {@link java.util.Base64} encoded string in
	 * {@link _clientPublicKeySubstrings }.
	 * 
	 * @param clientPublicKeyFilename location of file containing
	 *                                {@link java.security.PublicKey}.
	 * @return boolean indicating success or failure.
	 */
	private boolean initClientPublicKey(String clientPublicKeyFilename) {
	...
	private boolean initCredentials(String credentialsFilename) {
		...
		String encryptedCredentials = buffer.toString().trim();
		if (encryptedCredentials.toString().isBlank()) {
			System.err.println("error: credentials file " + credentialsFilename + " has no data");
			return false;
		}

		String credentials = decryptData(encryptedCredentials, _clientPrivateKey);
		if (credentials == null) {
	 ...
	 * @return boolean indicating success or failure.
	 */
	private boolean initServerPublicKey(String serverPublicKeyFilename) {
	...
	private boolean setPublicIpOnServer(String publicIp) {
		...
		if (!buildServerMessageData(requestData, getRandom(), getRandom(), publicIp)) {
			System.err.println("error: unable to build message data");
			return false;
		}
